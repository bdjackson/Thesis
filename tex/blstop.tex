...
% MC event weight - done
% pile up weight - done
% lumi*cross section weight - done
% lepton SF - done
% trigger SF - done
% b tag SF - done
% Z k factor - done
% ttbar pt reweght - done

% Cuts
% BMINUSL_CUT_GRL                    - done
% BMINUSL_CUT_INCOMPLETE_EVENT       - done
% BMINUSL_CUT_LAR_ERROR              - done
% BMINUSL_CUT_TILE_ERROR             - done
% BMINUSL_CUT_TILE_HOT_SPOT          - done
% BMINUSL_CUT_TILE_TRIP              - done
% BMINUSL_CUT_BAD_JET_VETO           - done
% BMINUSL_CUT_CALO_PROBLEM_JET       - done
% BMINUSL_CUT_PRIMARY_VERTEX         - done
% BMINUSL_CUT_BAD_MUON               - done
% BMINUSL_CUT_COSMIC_MUON_VETO       - done
% BMINUSL_CUT_HFOR                   - done
% BMINUSL_CUT_MC_OVERLAP             - done
% BMINUSL_CUT_GE_2_BASELINE_LEPTONS  - done
% BMINUSL_CUT_GE_2_SIGNAL_LEPTON     - done
% BMINUSL_CUT_OS_LEP                 - done
% BMINUSL_CUT_TRIGGER                - done
% BMINUSL_CUT_PHASE                  - done
% BMINUSL_CUT_TRIGGER_MATCH          - done
% BMINUSL_CUT_GE_2_B_JET             - done
% BMINUSL_CUT_BL_PAIRING             - done
% BMINUSL_CUT_ZVETO                  - done
...
As the value of $Br(\tilde{t} \to eb)$ increases (represented by moving toward
...
Similarly, as the value of $Br(\tilde{t} \to \mu b)$ increases (represented by
...
Finally, as the branching ratio of the stop to $\tau b$ decay increases,
represented by moving toward the upper right corner of the plots, the number of
expected events with two light leptons decreases.
The nominal di-stop branching fractions of 0.25, 0.25, and 0.50 for the $eebb$,
$\mu\mu bb$, $e\mu bb$ channels respectively.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{figs/blstop/branching_fractions.png}
  \caption{Branching fraction to each di-lepton flavor channel depending on the
    branching ratios of the stop.
  }
...
\begin{figure}[ht]
  ...
  \caption{Scaling factors which need to be applied to events depending on the
    target stop branching ratios.
  }
  \label{fig:flavor_scaling_sf}
\end{figure}

For each point on the branching ratio plane, the SR which provided the best
expected sensitivity, as measured by the lowest expected $CL_S$ value, is
chosen.
The expected and observed limits are shown in Figure \ref{fig:limit_contours}.
...
The final limit on the stop mass is shown in Figure~\ref{fig:mass_limit_obs}.
This plot shows the 95\% confidence limit~(CL) on the mass obtained by choosing
