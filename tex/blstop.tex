...
as described in \Cref{sec:electrons,sec:muons,sec:jets}.
...
\begin{table}[ht]
  ...
  \centering{
    \begin{tabular}{cc}
      ...
      Final state & Trigger \\
      \midrule
      \multirow{2}{*}{$eebb$}      &  \texttt{EF\_e24vhi\_medium1} \\[1ex]
                                   &  \texttt{EF\_e60\_medium1}    \\
      \midrule
      \multirow{2}{*}{$\mu\mu bb$} &  \texttt{EF\_mu24i\_tight}    \\[1ex]
                                   &  \texttt{EF\_mu36\_tight}     \\
      \midrule
      \multirow{4}{*}{$e\mu bb$}   &  \texttt{EF\_e24vhi\_medium1} \\[1ex]
                                   &  \texttt{EF\_e60\_medium1}    \\[1ex]
                                   &  \texttt{EF\_mu24i\_tight}    \\[1ex]
                                   &  \texttt{EF\_mu36\_tight}     \\
      \bottomrule
    \end{tabular}
  }
...
\begin{table}[bh]
  ...
  \centering{
    \begin{tabular}{c|cc}
      ...
      & & $\nicefrac{\pt^\mathrm{cone20}}{\pt} < 0.1$ \\ [2ex]
      \texttt{EF\_e60\_medium1} & $\pt^{e} \geq 60 \GeV$ & -- \\ [2ex]
      \texttt{EF\_mu24i\_tight}
      & $\pt^{e} \geq 24 \GeV$
      & $\nicefrac{\pt^\mathrm{cone20}}{\pt} < 0.12$ \\ [2ex]
...
% TODO switch from bottom to top version of these figures in final version
...
% TODO switch from bottom to top version of these figures in final version
  ...
  \caption{
    Efficiency of simulated stop events passing each single lepton
...
% TODO switch from bottom to top version of these figures in final version
    ...
    % \includegraphics[width=0.47\textwidth, clip=true, trim=0 0 1cm 0]
...
\begin{description}
  ...
  %%
    % TODO check if this new line is needed for final version
    % \\
...
% TODO switch from bottom to top version of these figures in final version
  ...
  \caption[
    ...
    % events are required to have at least two $b$-tagged jets and two
...
\begin{table}[ht]
  \caption[
    The number of expected signal events pasing each of the signal
    for stop masses of 500~\GeV, 800~\GeV, and 1000~\GeV.
  ]{
    The number of expected signal events pasing each of the signal
    for stop masses of 500~\GeV, 800~\GeV, and 1000~\GeV.
...
\begin{table}[ht]
  \caption[
    Summary of signal, control, and validation regions used for this
    analysis.
  ]{
    Summary of signal, control, and validation regions used for this
    analysis.
  ...
  \centering{
    \begin{tabular}{l|ccccc}
      \toprule
      Region &
      \multirow{2}{*}{$\MBL^0$ [\GeV]} &
      \multirow{2}{*}{\HT [\GeV]} &
      \MET &
      \multirow{2}{*}{\MBLASYM} &
      $Z$ window \\
      %%
      & & & significance [$\GeV^{1/2}$] & &
      $Z$ window \\
      \midrule
      SR~400   & $\geq 400$  & $\geq 1100$ & --       & $\leq 0.2$ & Veto   \\[1ex]
      SR~600   & $\geq 600$  & $\geq 1100$ & --       & $\leq 0.2$ & Veto   \\
...
\begin{table}[bh]
  \caption[
    Partial summary of background samples and their cross sections used
    in this analysis.
  ]{
    Partial summary of background samples and their cross sections used
    in this analysis.
    For each di-lepton flavor channel, samples are generated with requirements
    on the \pt\ of the $Z$ boson.
    For the inclusive \ZGAMMAJETS\ sample, only events with
    $\pt^{Z} \le 40 \GeV$ were used.
    For each combination of di-lepton flavor and \pt\ requirement, three
    samples are generated, with filters applied on the three jet flavors 
    different lepton
    flavor channels and filters applied on the jet flavor.
    The different filter efficiencies, lead to different effective
    luminosities.
...
\begin{table}[p]
  \caption[
    Summary of other background samples and their cross sections used in
    this analysis.
  ]{
    Summary of other background samples and their cross sections used in
    this analysis.
    The $W \rightarrow \ell\nu$ and $Z \rightarrow \ell\ell$ processes have 
...
%% ~~moved up a bit~~ \begin{table}[p]
%% ~~moved up a bit~~   \caption[
%% ~~moved up a bit~~     Partial summary of background samples and their cross sections used
%% ~~moved up a bit~~     in this analysis.
%% ~~moved up a bit~~   ]{
%% ~~moved up a bit~~     Partial summary of background samples and their cross sections used
%% ~~moved up a bit~~     in this analysis.
%% ~~moved up a bit~~     For each di-lepton flavor channel, samples are generated with requirements
%% ~~moved up a bit~~     on the \pt\ of the $Z$ boson.
%% ~~moved up a bit~~     For the inclusive \ZGAMMAJETS\ sample, only events with
%% ~~moved up a bit~~     $\pt^{Z} \le 40 \GeV$ were used.
%% ~~moved up a bit~~     For each combination of di-lepton flavor and \pt\ requirement, three
%% ~~moved up a bit~~     samples are generated, with filters applied on the three jet flavors 
%% ~~moved up a bit~~     different lepton
%% ~~moved up a bit~~     flavor channels and filters applied on the jet flavor.
%% ~~moved up a bit~~     The different filter efficiencies, lead to different effective
%% ~~moved up a bit~~     luminosities.
%% ~~moved up a bit~~     The left column shows the effective luminosity for the sample generated
%% ~~moved up a bit~~     with a $b$-filter.
%% ~~moved up a bit~~     The middle column represents the sample with a $c$-filter and $b$-veto.
%% ~~moved up a bit~~     The right column shows the effective luminosity of the sample generated
%% ~~moved up a bit~~     with a veto on both $b$- and $c$-quarks.
%% ~~moved up a bit~~   }
%% ~~moved up a bit~~   \label{tab:background_grouping}
%% ~~moved up a bit~~   \centering{
%% ~~moved up a bit~~     \begin{tabular}{c|cccccc}
%% ~~moved up a bit~~       \toprule
%% ~~moved up a bit~~       Grouping                      & Process                                          & Cross-section [pb]                    & \multicolumn{3}{c}{Luminosity [$\mathrm{fb}^{-1}$]} & Generator \\
%% ~~moved up a bit~~       \midrule
%% ~~moved up a bit~~       \TTBAR                        & \TTBAR                                           & 253                                   & \multicolumn{3}{c}{727.4}                                                                                 & \powheg+\pythia \\
%% ~~moved up a bit~~       \midrule
%% ~~moved up a bit~~       \multirow{3}{*}{Single top}   & $t$-channel                                      & 25.8                                  & \multicolumn{3}{c}{320}                                                                                   & \acermc+\pythia \\
%% ~~moved up a bit~~                                     & $s$-channel                                      & 1.64                                  & \multicolumn{3}{c}{330}                                                                                   & \powheg+\pythia \\
%% ~~moved up a bit~~                                     & $Wt$-channel                                     & 2.15                                  & \multicolumn{3}{c}{4200}                                                                                  & \powheg+\pythia \\
%% ~~moved up a bit~~       \midrule
%% ~~moved up a bit~~       \multirow{15}{*}{\ZGAMMAJETS} & $Z \rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$  & 1110                                  & 110                                                 & 9                           & 6                     & \sherpa  \\ [1ex]
%% ~~moved up a bit~~                                     & $Z \rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$  & \multirow{2}{*}{70.5}                 & \multirow{2}{*}{110}                                & \multirow{2}{*}{22}         & \multirow{2}{*}{30}   & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                     & $p_{T}^{Z} \in [40 ,70] \GeV$                    & & & & &  \\ [1ex]
%% ~~moved up a bit~~                                     & $Z \rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$  & \multirow{2}{*}{29.5}                 & \multirow{2}{*}{510}                                & \multirow{2}{*}{90}         & \multirow{2}{*}{110}  & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                     & $p_{T}^{Z} \in [70 ,140] \GeV$                   & & & & &  \\ [1ex]
%% ~~moved up a bit~~                                     & $Z \rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$  & \multirow{2}{*}{3.99}                 & \multirow{2}{*}{470}                                & \multirow{2}{*}{240}        & \multirow{2}{*}{250}  & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                     & $p_{T}^{Z} \in [140,280] \GeV$                   & & & & &  \\ [1ex]
%% ~~moved up a bit~~                                     & $Z \rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$  & \multirow{2}{*}{0.24}                 & \multirow{2}{*}{680}                                & \multirow{2}{*}{480}        & \multirow{2}{*}{360}  & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                     & $p_{T}^{Z} \in [280,500] \GeV$                   & & & & &  \\ [1ex]
%% ~~moved up a bit~~                                     & $Z \rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$  & \multirow{2}{*}{$1.3 \times 10^{-2}$} & \multirow{2}{*}{5700}                               & \multirow{2}{*}{1700}       & \multirow{2}{*}{7000} & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                     & $p_{T}^{Z} \ge 500 \GeV$                         & & & & &  \\ [1ex]
%% ~~moved up a bit~~                                     & DY $\rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$ & \multirow{2}{*}{92.1}                 & \multicolumn{3}{c}{\multirow{2}{*}{50}}                                                                   & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                     & $m_{Z/\gamma^{*}} \in [8 ,15] \GeV$              & & & & &  \\ [1ex]
%% ~~moved up a bit~~                                     & DY $\rightarrow \ell\ell (ee, \mu\mu, \tau\tau)$ & \multirow{2}{*}{279}                  & \multicolumn{3}{c}{\multirow{2}{*}{50}}                                                                   & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                     & $m_{Z/\gamma^{*}} \in [15,40] \GeV$              & & & & &  \\
%% ~~moved up a bit~~       \bottomrule
%% ~~moved up a bit~~     \end{tabular}
%% ~~moved up a bit~~   }
%% ~~moved up a bit~~ \end{table}
%% ~~moved up a bit~~ 
%% ~~moved up a bit~~ \begin{table}[p]
%% ~~moved up a bit~~   \caption[
%% ~~moved up a bit~~     Summary of other background samples and their cross sections used in
%% ~~moved up a bit~~     this analysis.
%% ~~moved up a bit~~   ]{
%% ~~moved up a bit~~     Summary of other background samples and their cross sections used in
%% ~~moved up a bit~~     this analysis.
%% ~~moved up a bit~~     The $W \rightarrow \ell\nu$ and $Z \rightarrow \ell\ell$ processes have 
%% ~~moved up a bit~~     dedicated samples for each lepton flavor as indicated in the parentheses.
%% ~~moved up a bit~~     The $W$ samples are generated with filters on the jet flavor.
%% ~~moved up a bit~~     As in Table~\ref{tab:background_grouping}, the three columns under the
%% ~~moved up a bit~~     effective luminosity represent the three samples ($b$-filter, $c$-filter
%% ~~moved up a bit~~     and $b$-veto, veto on both $b$- and $c$-quarks).
%% ~~moved up a bit~~   }
%% ~~moved up a bit~~   \label{tab:background_grouping_other}
%% ~~moved up a bit~~   \centering{
%% ~~moved up a bit~~       \begin{tabular}{c|cccccc}
%% ~~moved up a bit~~         \toprule
%% ~~moved up a bit~~         Grouping &
%% ~~moved up a bit~~         Process &
%% ~~moved up a bit~~         Cross-section [pb] &
%% ~~moved up a bit~~         \multicolumn{3}{c}{Luminosity [$\mathrm{fb}^{-1}$]} &
%% ~~moved up a bit~~         Generator \\
%% ~~moved up a bit~~         \midrule
%% ~~moved up a bit~~         \multirow{35}{*}{Other}    & $\TTBAR\,W$                                         & 0.10                  & \multicolumn{3}{c}{3300 } & \madgraph+\pythia \\
%% ~~moved up a bit~~                                    & $\TTBAR\,Wj$                                        & $9.3 \times 10^{-2}$  & \multicolumn{3}{c}{3600 } & \madgraph+\pythia \\ [1ex]
%% ~~moved up a bit~~                                    & $\TTBAR\,Z$                                         & $6.8 \times 10^{-2}$  & \multicolumn{3}{c}{4400 } & \madgraph+\pythia \\
%% ~~moved up a bit~~                                    & $\TTBAR\,Zj$                                        & $8.7 \times 10^{-2}$  & \multicolumn{3}{c}{3400 } & \madgraph+\pythia \\ [1ex]
%% ~~moved up a bit~~                                    & $\TTBAR\,WW$                                        & $9.2 \times 10^{-4}$  & \multicolumn{3}{c}{11000} & \madgraph+\pythia \\ [1ex]
%% ~~moved up a bit~~                                    & $WW \rightarrow \ell\ell\nu\nu$                     & 5.30                  & \multicolumn{3}{c}{1400 } & \sherpa           \\
%% ~~moved up a bit~~                                    & $WW \rightarrow \ell\nu qq (e,\mu,\tau)$            & 7.3                   & \multicolumn{3}{c}{100  } & \sherpa           \\ [1ex]
%% ~~moved up a bit~~                                    & $WZ \rightarrow \ell\ell\ell\nu$                    & 9.74                  & \multicolumn{3}{c}{260  } & \sherpa           \\
%% ~~moved up a bit~~                                    & $WZ \rightarrow \ell\nu\nu\nu$                      & 1.40                  & \multicolumn{3}{c}{270  } & \sherpa           \\
%% ~~moved up a bit~~                                    & $WZ \rightarrow \ell\nu qq (e,\mu,\tau)$            & 1.9                   & \multicolumn{3}{c}{110  } & \sherpa           \\
%% ~~moved up a bit~~                                    & $WZ \rightarrow \ell\ell qq (ee, \mu\mu, \tau\tau)$ & 1.46                  & \multicolumn{3}{c}{110  } & \sherpa           \\
%% ~~moved up a bit~~                                    & $WZ \rightarrow \nu\nu qq$                          & 2.70                  & \multicolumn{3}{c}{64   } & \sherpa           \\ [1ex]
%% ~~moved up a bit~~                                    & $ZZ \rightarrow \ell\ell\nu\nu$                     & 0.49                  & \multicolumn{3}{c}{1700 } & \sherpa           \\
%% ~~moved up a bit~~                                    & $ZZ \rightarrow \ell\ell qq (ee, \mu\mu, \tau\tau)$ & 0.25                  & \multicolumn{3}{c}{120  } & \sherpa           \\
%% ~~moved up a bit~~                                    & $ZZ \rightarrow \nu\nu qq$                          & 1.74                  & \multicolumn{3}{c}{69   } & \sherpa           \\ [1ex]
%% ~~moved up a bit~~                                    & ggf $H \rightarrow WW$                              & 0.44                  & \multicolumn{3}{c}{2000 } & \powheg+\pythia 8 \\
%% ~~moved up a bit~~                                    & ggf $H \rightarrow ZZ$                              & $4.7 \times 10^{-2}$  & \multicolumn{3}{c}{2000 } & \powheg+\pythia 8 \\
%% ~~moved up a bit~~                                    & VBF $H \rightarrow WW$                              & $3.6 \times 10^{-2}$  & \multicolumn{3}{c}{17000} & \powheg+\pythia 8 \\
%% ~~moved up a bit~~                                    & VBF $H \rightarrow ZZ$                              & $3.8 \times 10^{-3}$  & \multicolumn{3}{c}{18000} & \powheg+\pythia 8 \\ [1ex]
%% ~~moved up a bit~~                                    & $WH \rightarrow W\ell\nu\ell\nu$                    & 0.15                  & \multicolumn{3}{c}{1000 } & \pythia 8         \\
%% ~~moved up a bit~~                                    & $WH \rightarrow W\ell\ell\nu\nu$                    & $1.7 \times 10^{-3}$  & \multicolumn{3}{c}{27000} & \pythia 8         \\ [1ex]
%% ~~moved up a bit~~                                    & $ZH \rightarrow Z\ell\nu\ell\nu$                    & $8.9 \times 10^{-3}$  & \multicolumn{3}{c}{2000 } & \pythia 8         \\
%% ~~moved up a bit~~                                    & $ZH \rightarrow Z\ell\ell\nu\nu$                    & $1.0 \times 10^{-2}$  & \multicolumn{3}{c}{48000} & \pythia 8         \\ [1ex]
%% ~~moved up a bit~~                                    & $\TTBAR H \rightarrow \TTBAR WW$                    & $2.8 \times 10^{-2}$  & \multicolumn{3}{c}{7000 } & \pythia 8         \\ [1ex]
%% ~~moved up a bit~~                                    & $W \rightarrow \ell\nu (e, \mu, \tau)$              & 11000                 & 100                       & 17                   & 4                    & \sherpa \\ [1ex]
%% ~~moved up a bit~~                                    & $W \rightarrow \ell\nu (e, \mu, \tau)$              & \multirow{2}{*}{653}  & \multirow{2}{*}{44}       & \multirow{2}{*}{7}   & \multirow{2}{*}{30}  & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                    & $p_\mathrm{T}^{W} \in [40,70] \GeV$                 &                       &                           &                      &                      & \\ [1ex]
%% ~~moved up a bit~~                                    & $W \rightarrow \ell\nu (e, \mu, \tau)$              & \multirow{2}{*}{251}  & \multirow{2}{*}{160}      & \multirow{2}{*}{54}  & \multirow{2}{*}{24}  & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                    & $p_\mathrm{T}^{W} \in [70,140] \GeV$                &                       &                           &                      &                      & \\ [1ex]
%% ~~moved up a bit~~                                    & $W \rightarrow \ell\nu (e, \mu, \tau)$              & \multirow{2}{*}{31.2} & \multirow{2}{*}{460}      & \multirow{2}{*}{260} & \multirow{2}{*}{80}  & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                    & $p_\mathrm{T}^{W} \in [140,280] \GeV$               &                       &                           &                      &                      & \\ [1ex]
%% ~~moved up a bit~~                                    & $W \rightarrow \ell\nu (e, \mu, \tau)$              & \multirow{2}{*}{1.84} & \multirow{2}{*}{590}      & \multirow{2}{*}{420} & \multirow{2}{*}{360} & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                    & $p_\mathrm{T}^{W} \in [280,500] \GeV$               &                       &                           &                      &                      & \\ [1ex]
%% ~~moved up a bit~~                                    & $W \rightarrow \ell\nu (e, \mu, \tau)$              & \multirow{2}{*}{0.10} & \multirow{2}{*}{890}      & \multirow{2}{*}{360} & \multirow{2}{*}{140} & \multirow{2}{*}{\sherpa} \\
%% ~~moved up a bit~~                                    & $p_\mathrm{T}^{W} \ge 500 \GeV$                     &                       &                           &                      &                      &  \\
%% ~~moved up a bit~~         \bottomrule
%% ~~moved up a bit~~       \end{tabular}
%% ~~moved up a bit~~   }
%% ~~moved up a bit~~ \end{table}
...
%% ~~moved above~~ % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
%% ~~moved above~~ \begin{table}[t]
%% ~~moved above~~   \caption[
%% ~~moved above~~     The observed and expected event yields in the CRs and VRs. The
%% ~~moved above~~     expected event yields are shown before and after a fit to the data in
%% ~~moved above~~     the CRs.
%% ~~moved above~~   ]{
%% ~~moved above~~     The observed and expected event yields in the CRs and VRs. The
%% ~~moved above~~     expected event yields are shown before and after a fit to the data in
%% ~~moved above~~     the CRs.
%% ~~moved above~~     The fitted background yields in the CRs match the observed
%% ~~moved above~~     number of events in data by construction.
%% ~~moved above~~   }
%% ~~moved above~~   \label{tab:bkg_only_fit_results}
%% ~~moved above~~   %
%% ~~moved above~~   \begin{center}
%% ~~moved above~~     \begin{tabular}{lrrrrrr}
%% ~~moved above~~       \toprule
%% ~~moved above~~                          & Top CR           & $Z$ CR          & Top VR 1       & Top VR 2      & Top VR 3        & $Z$ VR          \\
%% ~~moved above~~       \midrule
%% ~~moved above~~       Observed           & $369$            & $327$           & $645$          & $606$         & $67$            & $101$           \\
%% ~~moved above~~       \midrule
%% ~~moved above~~       Fitted background  & $369   \pm 19$   & $327  \pm 18$   & $690  \pm 50$  & $630 \pm 40$  & $72   \pm 5$    & $130  \pm 60$   \\
%% ~~moved above~~       \midrule
%% ~~moved above~~       Fitted \TTBAR      & $346   \pm 19$   & $9.1  \pm 0.7$  & $600  \pm 40$  & $497 \pm 35$  & $54   \pm 5$    & $2.99 \pm 0.24$ \\[1ex]
%% ~~moved above~~       Fitted \ZGAMMAJETS & $3.2   \pm 0.5$  & $309  \pm 18$   & $63   \pm 5$   & $64  \pm 5$   & $1.5  \pm 0.8$  & $120  \pm 60$   \\[1ex]
%% ~~moved above~~       Single top         & $16.7  \pm 2.0$  & $0.83 \pm 0.09$ & $23.0 \pm 2.6$ & $56  \pm 6$   & $14.1 \pm 1.9$  & $0.32 \pm 0.04$ \\[1ex]
%% ~~moved above~~       Other              & $2.83  \pm 0.27$ & $8.64 \pm 1.0$  & $4.7  \pm 0.4$ & $8.2 \pm 0.8$ & $2.03 \pm 0.27$ & $6.4  \pm 0.7$  \\
%% ~~moved above~~       \midrule
%% ~~moved above~~       Input SM           & $330$            & $230$           & $614$          & $557$         & $66$            & $93$            \\
%% ~~moved above~~       \midrule
%% ~~moved above~~       Input \TTBAR       & $310$            & $8.2$           & $543$          & $447$         & $49$            & $2.7$           \\[1ex]
%% ~~moved above~~       Input \ZGAMMAJETS  & $2.2$            & $220$           & $44$           & $45$          & $1.1$           & $83$            \\[1ex]
%% ~~moved above~~       Input single top   & $17$             & $0.8$           & $23$           & $57$          & $14$            & $0.30$          \\[1ex]
%% ~~moved above~~       Input other        & $2.8$            & $8.6$           & $4.7$          & $8.2$         & $2.0$           & $6.40$          \\
%% ~~moved above~~       \midrule
%% ~~moved above~~       $B-L$ stop (500~\GeV)  & $1.3$        & $0.03$          & $5.1$          & $2.4$         & $10.7$          & $3.8$           \\[1ex]
%% ~~moved above~~       $B-L$ stop (800~\GeV)  & $< 0.01$     & $< 0.01$        & $< 0.01$       & $< 0.01$      & $0.6$           & $0.02$          \\[1ex]
%% ~~moved above~~       $B-L$ stop (1000~\GeV) & $< 0.01$     & $< 0.01$        & $< 0.01$       & $< 0.01$      & $0.1$           & $< 0.01$        \\
%% ~~moved above~~       \bottomrule
%% ~~moved above~~     \end{tabular}
%% ~~moved above~~   \end{center}
%% ~~moved above~~ \end{table}

%% ~~moved above~~ \begin{figure}[ht]
%% ~~moved above~~   \centering
%% ~~moved above~~   \includegraphics[width=\textwidth]{figs/blstop/histpull_CR_detailed.pdf}
%% ~~moved above~~   \caption[
%% ~~moved above~~     The number of observed and expected events in the CRs,
%% ~~moved above~~     broken down by flavor channel and for each di-lepton flavor channel.
%% ~~moved above~~     The deviation of that channel's prediction from the observed number of
%% ~~moved above~~     events divided by the uncertainty in the prediction is also shown.
%% ~~moved above~~   ]{
%% ~~moved above~~     The number of observed and expected events in the CRs,
%% ~~moved above~~     broken down by flavor channel and for each di-lepton flavor channel.
%% ~~moved above~~     The uncertainty band includes the statistical uncertainty as well as the
%% ~~moved above~~     systematic uncertainty (described in Section~\ref{sec:systematics}).
%% ~~moved above~~     The deviation of that channel's prediction from the observed number of
%% ~~moved above~~     events divided by the uncertainty in the prediction is also shown.
%% ~~moved above~~     The normalization of the background yields are determined
%% ~~moved above~~     by fitting the \TTBAR\ and \ZGAMMAJETS\ backgrounds to the observed
%% ~~moved above~~     data in the two CRs, so the Top CR and $Z$ CR bins have perfect agreement
%% ~~moved above~~     by construction.
%% ~~moved above~~   }
%% ~~moved above~~   \label{fig:pull_dist_cr}
%% ~~moved above~~ \end{figure}
%% ~~moved above~~ 
%% ~~moved above~~ \begin{figure}[ht]
%% ~~moved above~~   \centering
%% ~~moved above~~   \includegraphics[width=\textwidth]{figs/blstop/histpull_VR_detailed.pdf}
%% ~~moved above~~   \caption[
%% ~~moved above~~     The number of observed and expected events in the VRs,
%% ~~moved above~~     broken down by flavor channel and for each di-lepton flavor channel.
%% ~~moved above~~     The deviation of that channel's prediction from the observed number of
%% ~~moved above~~     events divided by the uncertainty in the prediction is also shown.
%% ~~moved above~~   ]{
...
%% ~~moved above~~     events divided by the uncertainty in the prediction is also shown.
%% ~~moved above~~     The normalization of the background yields are determined
%% ~~moved above~~     by fitting the \TTBAR\ and \ZGAMMAJETS\ backgrounds to the observed data in
%% ~~moved above~~     the two CRs.
%% ~~moved above~~   }
%% ~~moved above~~   \label{fig:pull_dist_vr}
%% ~~moved above~~ \end{figure}
...
% TODO switch from bottom to top version of these figures in final version
    ...
    \includegraphics[width=0.47\textwidth]{figs/blstop/vr_top_3_mbl_0.pdf}
  }
  \subbottom[$Z$ VR]{
% TODO switch from bottom to top version of these figures in final version
    % \includegraphics[width=0.48\textwidth]{figs/blstop/vr_Z_mbl_0.pdf}
    \includegraphics[width=0.47\textwidth]{figs/blstop/vr_Z_mbl_0.pdf}
  }
  \caption[
...
% TODO switch from bottom to top version of these figures in final version
    ...
    \includegraphics[width=0.47\textwidth]{figs/blstop/vr_top_3_ht_signal.pdf}
  }
  \subbottom[$Z$ VR]{
% TODO switch from bottom to top version of these figures in final version
    % \includegraphics[width=0.48\textwidth]{figs/blstop/vr_Z_ht_signal.pdf}
    \includegraphics[width=0.47\textwidth]{figs/blstop/vr_Z_ht_signal.pdf}
  }
  \caption[
    The \HT\ distribution in Top VR 3 (left) and $Z$ VR (right).
  ]{
    The \HT\ distribution in Top VR 3 (left) and $Z$ VR (right).
    The Standard Model background prediction is shown after setting the
    normalization of the \TTBAR\ and \ZGAMMAJETS\ backgrounds based on the
...
\centering{
  ...
  \toprule
    ...
    Luminosity                   & 1      & 1  \\
    \midrule                                   
    \HT\ extrapolation           & 19     & 20 \\[1ex]
    MC statistical               & 13     & 23 \\[1ex]
    CR statistical               & 3      & 3  \\[1ex]
    $Wt$ cross section           & 2      & 2  \\[1ex]
    Other theory                 & 1      & 2  \\
    \bottomrule
    \end{tabular}
}
...
\begin{table}[p]
  \caption[
    The event and object kinematics for the two events passing the
    signal region selection.
  ]{
    The event and object kinematics for the two events passing the
    signal region selection.
    The first event passes the SR~400 selection
...
\begin{figure}[ht]
  ...
  \caption[
    ...
    be set.
    %% {
    %%   \color{red} TODO remake with \atlas\ Internal on plots and add marker
    %%   where tested stop masses are
    %% }
  }
