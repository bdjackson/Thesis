...
\begin{table}[ht]
  \caption{Partial summary of background samples and their cross sections used in this
    analysis except for the \ZGAMMAJETS\ background (summarized in Tab.
    \ref{tab:background_grouping_z}) and the "other" category (summarized in
    Tab. \ref{tab:background_grouping_other}))
  }
  ...
  \centering{
    \begin{tabular}{c|cccc}
      ...
      Process &
      Cross-section [pb] &
      Luminosity [$\mathrm{fb}^{-1}$] &
      Generator \\
      \midrule
      \TTBAR                       & \TTBAR                    & 253  & 727.4 & \powheg+\pythia \\
      \midrule
      \multirow{3}{*}{Single top}  & Single top ($t$-channel)  & 25.8 & 320   & \acermc+\pythia \\
                                   & Single top ($s$-channel)  & 1.64 & 330   & \powheg+\pythia \\
                                   & Single top ($Wt$-channel) & 2.15 & 4200  & \powheg+\pythia \\
      \bottomrule
    \end{tabular}
  }
\end{table}
...
\begin{table}[ht]
\caption{Summary of the \ZGAMMAJETS\ background samples and their cross sections used
  in this analysis. The \ZGAMMAJETS\ samples are sliced by the \pt\ of the $Z$
  boson. For the inclusive sample, only events with $\pt^{Z} \le 40 \GeV$ were
  used. For each \pt\ slice, three samples were generated, with different
  filters applied on the jet flavor. The three samples are generated with
  approximately the same cross section, but the different filter efficiencies
  lead to different effective luminosities. The three columns under the
  effective luminosity represent the three samples. The left column shows the
  effective luminosity for the sample generated with a $b$-filter. The middle
  column represents the sample with a $c$-filter and $b$-veto. The right column
  shows the effective luminosity of the sample generated with a veto on both
  $b$ and $c$ quarks.
}
...
\begin{table}[ht]
  \caption{Summary of other background samples and their cross sections used in this
    analysis. The $W$ samples are generated with filters on the the jet flavor.
    As in Tab. \ref{tab:background_grouping_z},  The three columns under the
    effective luminosity represent the three samples (b-filter, c-filter and
    b veto, veto on both b and c quarks).
  }
