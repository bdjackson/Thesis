...
\begin{figure}[th]
  \centering{
    ...
    \caption{
      ...
      The mass of each particles are given in parentheses.
    }
...
% in a theory called quantum electrodynamics (QED).
...
The gluons of QCD are allowed to interact with each other, while the photon of
...
To show this, it is useful to consider the SM Lagrangian, ignoring the
QCD part,
\begin{equation}
  \mathcal{L} =
  - \frac{1}{4} W_{\mu\nu}^{a} W_{a}^{\mu\nu}
...
Expanding the scalar field $\Phi$ around its minimum $\Phi_0$, one gets
\begin{equation}
...
Dark energy, on the other hand, opposes the force of gravity and appears to
...
In order to construct a minimal supersymmetric Standard Model (MSSM), the SM
...
% corresponding gauginos mix to form the
...
\begin{table}[th]
  \caption[
    ...
    The neutrino sector constrains all but one of the R-parity violating
...
\begin{figure}[p]
  \centering{
    \subbottom[Stop branching ratio]{
      ...
      {figs/theory/StopBranchingRatiosVsMixingAngle.pdf}
